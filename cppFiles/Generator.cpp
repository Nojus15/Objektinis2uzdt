...
Generator::Generator() : file_name("studentai.txt")
{
}
...
void Generator::set_file_name(std::string name)
{
    if (!name.empty())
        file_name = name;
}
void Generator::set_nd_count(int nd) { ndCount = nd; }
void Generator::set_stud_count(int stud) { studCount = stud; }
void Generator::genFile()
{
    ...
    outputas << left << setw(7) << "Egz" << std::endl;
    for (int i = 0; i < studCount; i++)
    {
        outputas << genStudentStringStream(ndCount).str();
    }
    std::ofstream out_f(file_name);
    out_f << outputas.rdbuf();
    out_f.close();
    cout << studCount << " irasu failo generavimo laikas: " << genTimer.Stop_Return() << " s" << endl;
}
...
int Generator::generateRandomMark()
{

    return randomInt(1, 10);
}
std::stringstream Generator::genStudentStringStream(int ndCount)
{
    ...
    stud << std::left << std::setw(7) << randomInt(1, 10) << std::endl;
    return stud;
}
...
std::string Generator::randFirstName()
{
    return firstNames[randomInt(0, firstNames.size() - 1)];
}
std::string Generator::randLastName()
{
